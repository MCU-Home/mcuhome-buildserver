...
**What it needs, and cannot create for itself.** A container runtime, a
directory holding a real SDK package and its ``index.json`` (built by
mcuhome-sdk's ``scripts/build_sdk_archive.py``), a network that reaches
the registries the environment image and the environment packages come
from, and the ``mcuhome`` command — this harness drives the real client,
which is the point of it. The wrapper checks each and says which one is
...
def server(*, sdk_dir: Path, state: Path, log: Path, token: str):
    ...
    argv = [
        ...
        "--server-token",
        token,
        "--no-server-publish-pair-file",
        # One directory holding packages of all three kinds, named in all
        # three keys: a kind is never looked for under another kind's.
        "--build-sdk-sources",
        str(sdk_dir),
        ...
        "--server-context-root",
        str(state),
        "--server-session-idle-timeout-seconds",
...
def check_the_build_succeeded(document: dict, code: int) -> None:
    ...
    say("the remote build succeeded")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        with server(sdk_dir=sdk_dir, state=state, log=log, token=token) as port:
            ...
            document, code, seconds = build(project, port=port, token=token, sdk_dir=sdk_dir)
        build_dir = project / "build" / DEVICE

        print("checks:", flush=True)
        check_the_build_succeeded(document, code)
    ...
    print("\nOK — one real remote build, and every check above ran for real.", flush=True)
    return 0
