...
**The build server is never itself the build environment, and it is not
the orchestrator either.** It admits a client, takes a build context,
picks the container that context pinned and hands the whole build to the
workbench — the same code a local `mcuhome device build` runs — then
tunnels the events and the artifacts back. What it owns is the session:
admission, the context's lifetime, invocation ids, replay, egress and
the verdict.
...
The whole protocol surface is real, and so is the build. A client can connect, negotiate, open a session,
...
| `--allow-environment` | `ghcr.io/mcu-home/build-container` | build-environment repository this server may run, without tag or digest; repeatable. Stating it **replaces** the default |
...
| `--max-sessions` | `4` | how many sessions may be open at once |
...
`--max-sessions` bounds how many sessions are open at once. Four
containers at `--container-memory` each is a number an operator has to
be able to lower on a machine that cannot feed them. Sizing it from real
load is a later version's job; a static one is what an operator can
reason about, and a dynamic one that guessed wrong would be a build
killed for arithmetic.
...
| `open-session` | `{"profile", "protocol_version", "context_format"}` | **answers**: admission — session id, lease, negotiated versions, and how this server executes builds (`container`). A version mismatch is a typed rejection at the door |
...
worth saying plainly which child that is: the `docker exec` client, and
SIGTERM does not reach the process inside the container — killing an
exec client never has, which is exactly why the contract has a
cooperative sentinel at all. What actually reaps a program that ignored
both is the container going away at `close-session`.

**Patched layers cost nothing to make writable.** The container's own
copy-on-write layer **is** the writable view §6.2 asks for: the image's trees are writable inside the container by
...
program, so no single version could be named here truthfully. What this server can build is a question for the
`capabilities` verb.
...
**That second half is open.** An App has no container runtime of its
own, and every session this server serves today runs in a container.
Building inside an App therefore needs an execution that unpacks a build
environment and runs the program in it directly — a machine-side
property, never a client's to ask for — and it does not exist yet.
...
What that leaves under test is exactly what this server is: which images
it will serve, what it makes of the result the orchestrator hands back,
...
| `backend.py` | this server's half of a build: which images it serves, the session's environment, invocation ids, the streams, the verdict |
| `container.py` | docker discovery, and the one seam every call to it goes through |
| `processes.py` | child processes: run one to completion, or spawn one and stream it |
| `abi.py` | the invocation ABI: the result document, read and judged |
| `events.py` | the invocation's NDJSON event stream: relay and replay |
| `artifacts.py` | what is not served, and the download archive |
